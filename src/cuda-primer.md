...
で、この`grid`とか`block`って何なのかというと、CUDAのスレッドの実行モデルを表しています。以下の図がCUDAにおけるスレッドの実行モデルで、グリッドが複数のブロックを持ち、それぞれのブロックは複数のスレッドを持ちます。`grid`や`block`で、生成するブロックの数や各ブロックの中のスレッドの数を指定しているわけですな。
...
で、キャッシュ・メモリとメイン・メモリでは読み書きに必要な時間が全然違う（下の図のようにキャッシュが速くてメイン・メモリが遅い）ので、このコードはとてもメモリ・アクセスの効率が悪い。
